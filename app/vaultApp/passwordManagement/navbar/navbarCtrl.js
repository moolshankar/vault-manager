app.controller("passwordNavbarCtrl", function ($scope,$rootScope,$mdDialog,$http,$state,$window,$location){
  if($rootScope.loggedInStatus != true){
        if($location.path() != '/credential'){
          $state.go('credential');
        }
        $mdDialog.show({
            templateUrl: '/vaultApp/passwordManagement/login/login.html',
    ...
    if($window.sessionStorage.getItem('userStatus') == 'New'){
        $mdDialog.show({
            templateUrl: '/vaultApp/passwordManagement/login/resetPassword.html',
            parent: angular.element(document.body),
            clickOutsideToClose:false,
            escapeToClose:false,
            fullscreen: false
