app.controller('pwdLoginCtrl', function ($scope,$http,$rootScope,$window,$state) {
    ...
    $scope.loginUser = function(){
      if($scope.user.name == '' || $scope.user.password == ''){
        ...
        $scope.errorMsg = "Username or password is blank.";
            ...
            $http.post($rootScope.appUrl+'/authUser',$scope.user).then(function(response){
            if(response.data.status == 'success'){
                $scope.formStatus = 'Initial';
                $rootScope.setSessionParams(response.data);
                $window.location.href = '/credential/dashboard';
            }
